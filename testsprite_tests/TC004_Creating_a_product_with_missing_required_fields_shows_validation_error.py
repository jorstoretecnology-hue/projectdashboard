...
async def run_test():
    ...
    try:
        ...
        # -> Click the login/submit button again to complete authentication and reach the dashboard (if the app responds).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/div[4]/div/div[2]/form/div[2]/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Try the alternate credentials from extra info: replace email and password inputs with jaomart01@gmail.com / U&yggD1G$s!uLW*t6 and submit the login form (click 'Entrar al Dashboard').
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[4]/div/div[2]/form/div/input').nth(0)
        ...
        # -> Navigate directly to the Inventory page (try /console/inventory) to locate product creation UI.
        await page.goto("http://localhost:3000/console/inventory", wait_until="commit", timeout=10000)
        
        # --> Assertions to verify final state
        frame = context.pages[-1]
        await expect(frame.locator('text=Please fill out all required fields').first).to_be_visible(timeout=3000)
        assert not await frame.locator('text=Invalid Product Missing Fields').first.is_visible()
