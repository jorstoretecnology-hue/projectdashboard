...
export function InvitationDialog({ open, onOpenChange, tenantId, tenantName }: InvitationDialogProps) {
    ...
    <Dialog open={open} onOpenChange={(val) => {
      ...
      <DialogContent className="sm:max-w-[450px] border-border/50 bg-card/95 backdrop-blur-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Cree un enlace de registro seguro para <span className="font-bold text-foreground">{tenantName}</span>.
          </DialogDescription>
        </DialogHeader>
        ...
        {!invitationLink ? (
          <form onSubmit={handleSubmit} className="space-y-6 pt-4">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="email" className="text-xs font-bold uppercase text-muted-foreground">Email del Invitado</Label>
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-muted-foreground mt-1">El Owner tendrá control total sobre este cliente específico.</p>
              </div>
            </div>

            <Button type="submit" className="w-full gap-2 shadow-lg shadow-primary/20" disabled={isSubmitting}>
          ...
          <div className="space-y-6 pt-4 animate-in zoom-in-95 duration-300">
            <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 space-y-3">
              <Label className="text-xs font-bold uppercase text-primary">Enlace de Invitación</Label>
              <div className="flex gap-2">
                <Input readOnly value={invitationLink} className="bg-background border-primary/20 text-xs font-mono" />
                <Button size="icon" variant="outline" onClick={copyToClipboard}>
                  {copied ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4" />}
                </Button>
              </div>
              <p className="text-[10px] text-muted-foreground">Comparte este link con el cliente. Expira en 7 días.</p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button variant="ghost" className="w-full gap-2 text-muted-foreground" onClick={() => setInvitationLink(null)}>
