...
export async function proxy(request: NextRequest) {
  ...
  if (role === 'SUPER_ADMIN') {
    ...
    const superAdminEntryPoint = ['/', '/onboarding', '/post-auth', '/auth/login']
    if (superAdminEntryPoint.includes(pathname)) {
    ...
    // Check de Onboarding: Si no tiene tenant, mandarlo a onboarding
    if (!tenantId && pathname !== '/onboarding' && !isPublic && !bypassOnboarding) {
      return NextResponse.redirect(new URL('/onboarding', request.url))
    }

    // Si ya tiene tenant y está en onboarding, mandarlo al dashboard
