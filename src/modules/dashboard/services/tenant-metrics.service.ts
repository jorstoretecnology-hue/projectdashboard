 ...
 * Usa React.cache() para deduplicar requests en la misma request.
 */
...
// Cache con React.cache para deduplicar en la misma request
export const getCachedMetrics = cache(async (tenantId: string) => {
  return tenantDashboardService.getMetrics(tenantId);
});
