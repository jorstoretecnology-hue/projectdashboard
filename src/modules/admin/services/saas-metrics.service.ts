...
export class SaasMetricsService {
  ...
  async getDashboardSnapshot() {
    ...
    const [tenantsRes, quotasRes] = await Promise.all([
      supabase
        .from('tenants')
        .select(
          'id, name, plan, industry_type, active_modules, branding, is_active, max_users, created_at, updated_at, custom_domain, tenant_modules(module_slug)',
        ),
    ...
    const tenants: TenantConfig[] = rawTenants.map((t) => {
      ...
      // Usar minúsculas para que coincida con MODULE_DEFINITIONS (en lowercase)
      const activeModules =
        tm && tm.length > 0 ? tm.map((m) => m.module_slug.toLowerCase()) : t.active_modules || [];
  ...
  async getTopConsumers(
    resource: 'maxInventoryItems' | 'maxCustomers',
    limit = 5,
  ): Promise<TopConsumer[]> {
  ...
  private async getQuotaData(): Promise<any[]> {
    ...
    const { data } = await supabase
      .from('tenant_quotas')
      .select('id, tenant_id, resource_key, current_usage, limit_override, updated_at');
    return data || [];
  }
}
