 ...
 * Vista analítica agregada de clientes por tenant.
 * Read-only, sin acciones.
...
export default function SuperAdminCustomersPage() {
  ...
  const customerStats = useMemo(() => {
    return tenants.map((tenant) => {
      ...
      return {
        ...
        plan: tenant.plan,
        totalCustomers: realStats?.customers ?? 0, // Métrica REAL
  ...
  const globalStats = useMemo(() => {
    const active = customerStats.filter((s) => s.isActive)
    return {
      totalCustomers: customerStats.reduce((sum, s) => sum + s.totalCustomers, 0),
      activeTenants: active.length,
      tenantsWithCustomers: customerStats.filter((s) => s.totalCustomers > 0).length,
    }
  }, [customerStats])
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Cartera Clientes</h1>
            <p className="text-muted-foreground">
              Métricas reales de clientes por tenant
            </p>
          </div>
        </div>
        ...
        <Card className="border-border/50 bg-card/50">
          ...
          <CardContent>
            {isLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{globalStats.tenantsWithCustomers}</div>
                <p className="text-xs text-muted-foreground mt-1">Tenants con cartera</p>
              </>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="border-border/50">
        <CardHeader>
          ...
          <CardDescription>
            Uso real de clientes desde Quota Engine
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
